...
Na_0 = 160.0 / C_ref_2 # adim
Cl_0 = 160.0 / C_ref_2 # adim
...
res = GalacticOptim.solve(prob,Optim.BFGS();cb=cb,maxiters=50000)
...
Na_predict = [ [ phi[3]([t,x],minimizers[3])[1] for x in xs] for t in ts_] 
Cl_predict = [ [ phi[4]([t,x],minimizers[4])[1] for x in xs] for t in ts_] 

using Plots, LaTeXStrings
p1 = plot(xs, H_predict * C_ref_1, title = L"$H^+ | M$");
p2 = plot(xs, OH_predict * C_ref_1, title = L"$OH^- | M$");
p3 = plot(xs, Na_predict * C_ref_2, title = L"$Na^+ | M$");
p4 = plot(xs, Cl_predict * C_ref_2, title = L"$Cl^- | M$");
plot(p1,p2,p3,p4)
